 ...
 *
 *  - sorts it using standard sort, noting the execution time;
 *
 *  - sorts it using YOUR sort implementation, also noting the
 *    execution time;
 *
...
main (int argc, char* argv[])
{
  ...
  /* Sort sequentially */
  keytype* A_std = newCopy (N, A_in);
  stopwatch_start (timer);
  standardSort (N, A_std);
  long double t_std = stopwatch_stop (timer);
  printf ("Standard: %Lg seconds ==> %Lg million keys per second\n",
	  t_std, 1e-6 * N / t_std);
  assertIsSorted (N, A_std);

  /* Sort, calling YOUR routine. */
  keytype* A_ins = newCopy (N, A_in);
  stopwatch_start (timer);
  insertionSort (N, A_ins);
  long double t_qs = stopwatch_stop (timer);
  printf ("Insertion sort: %Lg seconds ==> %Lg million keys per second\n",
	  t_qs, 1e-6 * N / t_qs);
  assertIsSorted (N, A_ins);
  assertIsEqual (N, A_ins, A_std);
